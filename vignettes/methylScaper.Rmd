...
visualizing methylation data. The package offers processing for
both single-cell and single-molecule data, and a common interface for
seriating and visualizing both data types through the generation of
representational methylation-state matrices. Users may also run the
package through a Shiny app which provide an interactive seriation
process through refinement and re-weighting.
...
`methylscaper` can handle the analysis of BS-seq (?) data processed
...
below demonstrates this using data from (citation for GSE109262),
obtained from
[here](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE109262). For
...
them with a sequence plot. To demonstrate this, we use the `day7`
...
PCA-based ordering, but any method supported by the `seriation`
package can be used. It can also perform a weighted ordering,
according to the methylation or accessibility status of a subset of
the data's columns.
...
The plot on the left corresponds to methylation data, and the right to
accessibility data. The colored portions represent the areas on each
read that are methylated/accessible.
...
code below reorders the first 50 reads with Hierarchical Clustering,
and generates a new sequence plot.

```{r}
orderObj$order1 <- refineFunction(orderObj, 1, 50, Method = "HC")
plotSequence(orderObj)
```
...
accessibility data,
and the HCG file contains CG site methylation data. These files can
