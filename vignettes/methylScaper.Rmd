...
The `plotSequence` function generates a sequence plot from an
`orderObject`, as seen in the Shiny app.

Finally, the package exports the `percent_C` and `proportionColor` functions for computing and plotting summary statistics.

## Example Analysis

The package also includes a small data set, titled `day7`, that can be used for
testing and demonstration. (describe where the data came from?) The
...
The `day7` object is a list containing sets of HCG and GCH
data. We can perform an ordering of the data with the `initialOrder`
function:

```{r, fig.height = 5, fig.width = 6, fig.align="center"}
orderObj <- initialOrder(input.GCH = day7$gch, input.HCG = day7$hcg,
Method = "PCA")
plot1.out <- plotSequence(orderObj)
```

We can then refine a subset of the data, and generate a new plot:

```{r, fig.height = 5, fig.width = 6, fig.align="center"}
orderObj$order1 <- refineFunction(orderObj, 1, 20)
plot2.out <- plotSequence(orderObj)
```
