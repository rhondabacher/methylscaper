...
`methylscaper` can also be installed via [GitHub](https://github.com/rhondabacher/methylscaper/). The R4.0 branch is current with the Bioconductor version and only requires R version >= 4.0.0 while 4.1 is still under development or for those who have not yet upgraded.
...
In the screenshot below, the data from GSE10926 data on chromosome 19 is ready for processing. 
...
When using methylscaper within R, rather than specifying all the files, simply point to a folder which contains two subfolders with the accessibility and endogenous methylation files. These subfolders must be named "acc" and "met", respectively.
...
For a reproducible example, we have provided three cells for download on [http://methylscaper.com/content/exampledata.html](http://methylscaper.com/content/exampledata.html), and below we run an example where we read the data directly from the URL's into R and use the subsetSC function. If downloading the files for these three cells, then the directions above should be followed by moving the files into subfolders named "acc" and "met".
...
is automatically high resolution, though you will still want to adjust the width and height to your liking.
