...
plotSequence <- function(orderObject, plotFAST=TRUE,
    ...
    input.GCH <- toClust[,seq(1,(ncol(toClust) / 2))]
    input.HCG <- toClust[,seq((ncol(toClust) / 2 + 1),ncol(toClust))]
    ...
    blankCOLS <- matrix(rep(0, nrow(input.HCG)*blankWidth), 
                    nrow=nrow(input.HCG), ncol=blankWidth)
    ...
    if(drawKey == TRUE) {
      blankROW <- matrix(rep(0, ncol(toPlot.fix)*12), 
                          nrow=12, ncol=ncol(toPlot.fix))
      blankROW[seq(5,8),seq(1,147)] <- 2 #147 is nucleosome
      blankROW[seq(5,8),seq((seq(1,147)+(ncol(input.HCG.fix) + ncol(blankCOLS))))] <- 2
      toPlot.fix <- rbind(blankROW,toPlot.fix)
    }
    # Plotting:
    par(xpd = FALSE, mar=c(2,2,2,1), mgp = c(0,0.5,0))
    image(t(toPlot.fix), col=mycols, axes=FALSE, breaks=VALS,
    ...
    plot1 <- round(ncol(input.HCG.fix)/ncol(toPlot.fix), 2) 
    ...
    axis(2, at = seq(y.axis.starting.point,1,
